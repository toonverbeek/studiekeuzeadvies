"""Shapes text with HarfBuzz and turns it into SVG path data. Used by
docs/art/build-marks.py; it does nothing on its own.
...
THE FONT FILE IS NOT IN THIS REPO, and the path is not guessed. See the note on
ALEGREYA_SANS_BOLD below: the marks are regenerated by hand once in a blue moon,
so a download step that you run knowingly beats a path that silently rots.
"""
import os
...
# The full family as Google Fonts ships it, hinted, 261 KB. Deliberately NOT
# the subsets in app/fonts/: those are made for next/og and they carry no
# hinting, and build-marks.py leans on the hinting to keep both counters of the
# S open when FreeType draws the 16 pixel tile of the favicon.
#
#   curl -L -o ~/AlegreyaSans-Bold.ttf \
#     https://github.com/google/fonts/raw/main/ofl/alegreyasans/AlegreyaSans-Bold.ttf
#   ALEGREYA_SANS_BOLD=~/AlegreyaSans-Bold.ttf python3 docs/art/build-marks.py
TTF = os.path.expanduser(os.environ.get("ALEGREYA_SANS_BOLD", ""))

if not TTF or not os.path.isfile(TTF):
    raise SystemExit(
        "Alegreya Sans Bold not found. Set ALEGREYA_SANS_BOLD to a copy of it:\n"
        "  curl -L -o ~/AlegreyaSans-Bold.ttf \\\n"
        "    https://github.com/google/fonts/raw/main/ofl/alegreyasans/"
        "AlegreyaSans-Bold.ttf\n"
        "  ALEGREYA_SANS_BOLD=~/AlegreyaSans-Bold.ttf python3 "
        "docs/art/build-marks.py\n"
        "The licence is app/fonts/OFL.txt. Needs: fontTools, uharfbuzz, Pillow."
    )
